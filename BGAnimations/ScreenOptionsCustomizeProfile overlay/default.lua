...
local number_entry= new_numpad_entry{
	Name= "number_entry",
	InitCommand= cmd(diffusealpha, 0; x,SCREEN_CENTER_X+150; y,SCREEN_CENTER_Y+120),
	value = LoadFont("Infinity 24px") .. {
	...
	cursor_draw= "first",
	prompt = LoadFont("_eurostile outline") .. {
		Name="prompt",
		InitCommand=cmd(xy,0,-96);
		OnCommand=cmd(zoom,0.75;shadowlength,1;skewx,-0.125;diffusebottomedge,color("#DDDDDD");strokecolor,Color.Outline);
...
char_list= {
	{setting= "1", display_name= "---"},
	{setting= "2", display_name= "No"},
	{setting= "3", display_name= "Yes"},
	{setting= "4", display_name= "Seeking"},
	{setting= "5", display_name= "Rather Not Say"},
}
...
local menu_x= SCREEN_CENTER_X-240
local value_x= 330
local fader
...
local function exit_screen()
	...
	SOUND:PlayOnce(THEME:GetPathS("", "clash"), true)
end
...
local function input(event)
	...
	if cursor_on_menu == "main" then
		if button == "Start" then
			...
			if item.item_type == "bool" then
			SOUND:PlayOnce(THEME:GetPathS("", "zap"), true)
			...
			elseif item.item_type == "number" then
				...
				number_entry.prompt_actor:playcommand(
					...
					SOUND:PlayOnce(THEME:GetPathS("", "_switch up"), true)
...
for i, item in ipairs(menu_items) do
	...
	args[#args+1]= Def.BitmapText{
		Name= "menu_" .. item.name, Font= "_eurostile outline",
	...
	if item.get then
		...
		local value_args= {
			...
			Def.BitmapText{
				Name= "val", Font= "Infinity 24px", Text= value_text,
				InitCommand= function(self)
					self:diffuse(Color.White)
...
args[#args+1]= LoadActor("giant bar (doubleres)") .. {
	Name= "fader", InitCommand= function(self)
		fader= self
		self:draworder(-20)
		self:x(SCREEN_CENTER_X-100)
